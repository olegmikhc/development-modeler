...
export function SharedModel({token}:{token:string}){
 useLanguage();const [model,setModel]=useState<FinancialModel|null>(null),[error,setError]=useState(''),[message,setMessage]=useState(''),[busy,setBusy]=useState(false),[canEdit,setCanEdit]=useState(false);
 ...
 const openCopy=()=>{if(!model)return;useModels.getState().importModel(copyModel(model));location.assign(appPath('/'))};
 const openOriginal=async()=>{if(!model)return;setBusy(true);const {data,error}=await supabase()!.from('financial_models').select('data').eq('id',model.id).single();const parsed=modelEnvelope.safeParse(data?.data);if(error||!parsed.success){setMessage(tr('Access is unavailable.'));setBusy(false);return}const store=useModels.getState(),existing=store.models.find(m=>m.id===model.id);if(existing)store.mergeVersions([{id:crypto.randomUUID(),name:'Before opening shared model',date:new Date().toISOString(),model:structuredClone(existing)}]);store.importModel(parsed.data as FinancialModel);location.assign(appPath('/'))};
 return model?<><div className="shared-toolbar"><div><strong>{model.name}</strong><small>{tr('View only · original model is protected')}</small></div><div className="row" style={{flexWrap:'wrap'}}><LanguageSwitch/><button className="btn" onClick={openCopy}>{tr('Make an editable copy')}</button>{canEdit?<button className="btn dark" disabled={busy} onClick={openOriginal}>{tr('Edit original')}</button>:<button className="btn" disabled={busy} onClick={request}>{tr('Request editing access')}</button>}<button className="btn" onClick={()=>check(model.id)}>{tr('Check access')}</button><a className="btn" href={appPath('/login/')} target="_blank" rel="noreferrer">{tr('Sign in')}</a></div>{message&&<p role="status">{message}</p>}<small>{tr('A copy is saved on this device. Sign in and save it to cloud to use it on other devices.')}</small></div><Report model={model}/></>:<div className="boot">{error||'Loading shared model…'}</div>;
}
