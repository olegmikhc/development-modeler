...
it('exports fourteen populated worksheets with live roll-forward formulas',async()=>{const m=demoModel();const w=await createWorkbook(m);expect(w.worksheets.length).toBe(14);const cash=w.getWorksheet('Cash Flow')!;expect(cash.getCell(`B${cash.rowCount}`).formula).toContain('B');expect(w.getWorksheet('Dashboard')!.getCell('B8').formula).toContain('MAX(0,-MIN');expect(modelEnvelope.safeParse(m).success).toBe(true);writeFileSync('artifacts/demo-model.json',JSON.stringify(m));await w.xlsx.writeFile('artifacts/demo-financial-model.xlsx');});
