...
try{
 const page=await browser.newPage();await page.goto(base+'/');await page.getByRole('button',{name:'Edit developer name'}).waitFor();
 ...
 await page.goto(base+'/shared/?token=test-only-token');await page.getByRole('button',{name:'Make an editable copy'}).waitFor();
